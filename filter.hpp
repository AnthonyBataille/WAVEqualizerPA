...
/**
 * @brief Generic IIR filter that can specialize as a peak-notch or shelving filter.
 * It features a computation of output sample via overload of operator()
...
/**
 * @brief Second order peaking-notch IIR filter defined via the following parameters:
 Bandwidth in Hz
 Gain
 Center Frequency in Hz
 Maxuimum frequency (Nyquist fequency) in Hz.
 */
...
protected:
	...
	/**
	 * @brief computes the feedback (a) and feedforward (b) filter coefficients given the input parameters.
	 */
	void compute_coefficients();
