...
bool WAVHandler::checkHeader() {
  ...
  if (wavFile.gcount() != sizeof fileTypeBlocID ||
      std::string(fileTypeBlocID, sizeof fileTypeBlocID) != "RIFF") {
    return false;
  }
  ...
  if (wavFile.gcount() != sizeof fileFormatID ||
      std::string(fileFormatID, sizeof fileFormatID) != "WAVE") {
    return false;
  }
  ...
  if (wavFile.gcount() != sizeof formatBlocID ||
      std::string(formatBlocID, sizeof formatBlocID) != "fmt ") {
    return false;
  }
  ...
  if (wavFile.gcount() != sizeof dataBlocID ||
      std::string(dataBlocID, sizeof dataBlocID) != "data") {
    return false;
  }
...
WAVHandler::WAVHandler(const std::wstring &filePath) { init(filePath); }
...
WAVHandler::WAVHandler()
    : wavFile(), wavFilePath(), bytesPerBloc(0U), dataChunkSize(0U),
      dataChunk(nullptr), dataCursor() {}
