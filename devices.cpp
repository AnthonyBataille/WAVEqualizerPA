...
void printPADevicesInfo() {
  ...
  for (int i = 0; i < numDev; ++i) {
    ...
    std::cout << "\t\t\tLow: " << deviceInfo->defaultLowInputLatency
              << ", high: " << deviceInfo->defaultHighInputLatency << std::endl;

    std::cout << "\t\tDefault output latency:" << std::endl;
    std::cout << "\t\t\tLow: " << deviceInfo->defaultLowOutputLatency
              << ", high: " << deviceInfo->defaultHighOutputLatency
              << std::endl;
    ...
    if (inputParameters.channelCount > 0) {
      std::cout << "Supported standard sample rates" << std::endl
                << "for half - duplex 16 bit " << inputParameters.channelCount
                << " channel input = " << std::endl;
      PrintSupportedStandardSampleRates(&inputParameters, nullptr);
    }
    if (outputParameters.channelCount > 0) {
      std::cout << "Supported standard sample rates" << std::endl
                << "for half - duplex 16 bit " << outputParameters.channelCount
    ...
    if (inputParameters.channelCount > 0 && outputParameters.channelCount > 0) {
      std::cout << "Supported standard sample rates" << std::endl
                << "for half - duplex 16 bit " << inputParameters.channelCount
                << " channel input, " << inputParameters.channelCount
                << " channel input = " << std::endl;
