...
bool errorHandlerPA(const PaError err) {
	if (err != paNoError) {
		...
		std::swprintf(err_text.data(), size + 1, L"PA error : %s", PaErrString.data());
...
bool GUI::closeWAVStream() {
	if (audioHandle.stream.isStarted) {
		if (!audioHandle.stream.stop()) {
			MessageBox(_hwnd, L"Unable to stop the stream.", L"Error", MB_ICONERROR | MB_OK);
			return false;
		}

		if (!audioHandle.stream.close()) {
			MessageBox(_hwnd, L"Unable to close the stream.", L"Error", MB_ICONERROR | MB_OK);
			return false;
		}
	}
	return true;
...
void GUI::createControls(HWND hwnd) {
	...
	_wavFileNameDialog.lpstrFilter = L"WAV Files (*.wav)\0*.wav\0All Files (*.*)\0*.*\0";
	_wavFileNameDialog.lpstrFile = _wavFileName.data();
	_wavFileNameDialog.nMaxFile = MAX_PATH;
	_wavFileNameDialog.Flags = OFN_EXPLORER | OFN_FILEMUSTEXIST | OFN_HIDEREADONLY;
	_wavFileNameDialog.lpstrDefExt = L"wav";

	HWND fileNameLabel = CreateWindow(L"Static", L"WAV file path:", WS_CHILD | WS_VISIBLE,
		vertLinePos + 10, 20, 100, 20, hwnd, (HMENU)FILE_NAME_LABEL_ID, NULL, NULL);
	
	_fileNameTextBox = CreateWindow(L"Edit", L"", WS_CHILD | WS_VISIBLE | WS_BORDER | ES_READONLY | ES_AUTOHSCROLL,
		vertLinePos + 10, 50, 170, 20, hwnd, (HMENU)FILE_NAME_TEXTBOX_ID, NULL, NULL);

	HWND openFileButton = CreateWindow(L"Button", L"Open File", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
		vertLinePos + 10, 80, 100, 20, hwnd, (HMENU)OPEN_FILE_BUTTON_ID, NULL, NULL);

	constexpr int horzLinePos{ 120 };
	HWND horizontalSeparator = CreateWindow(L"Static", L"", WS_CHILD | WS_VISIBLE | SS_ETCHEDHORZ,
		vertLinePos, horzLinePos, 200, 1, hwnd, (HMENU)HORZ_SEP_ID, NULL, NULL);

	HWND playButton = CreateWindow(L"Button", L"Play", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
		vertLinePos + 20, horzLinePos + 30, 70, 20, hwnd, (HMENU)PLAY_BUTTON_ID, NULL, NULL);

	HWND stopButton = CreateWindow(L"Button", L"Stop", WS_TABSTOP | WS_VISIBLE | WS_CHILD | BS_DEFPUSHBUTTON,
		vertLinePos + 100, horzLinePos + 30, 70, 20, hwnd, (HMENU)STOP_BUTTON_ID, NULL, NULL);
}
...
LRESULT CALLBACK GUI::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam) {
			...
			case OPEN_FILE_BUTTON_ID: {
				if (GetOpenFileName(&(pgui->_wavFileNameDialog))) {
					...
					if (!pgui->closeWAVStream()) {
						MessageBox(hwnd, L"Unable to close the previous stream.", L"Error", MB_ICONERROR | MB_OK);
					}
					if (!pgui->openWAVStream(pgui->_wavFileName)) {
						MessageBox(hwnd, L"Unable to open the stream.", L"Error", MB_ICONERROR | MB_OK);
					}
				}
				else {
					wchar_t error_msg[128];
			...
			case PLAY_BUTTON_ID: {
				if (!pgui->audioHandle.stream.isStarted) {
					if (!pgui->audioHandle.stream.start()) {
						...
						MessageBox(hwnd, L"Unable to start the stream.", L"Error", MB_ICONERROR | MB_OK);
					}
				}
			}
			break;
			case STOP_BUTTON_ID: {
				if (pgui->audioHandle.stream.isStarted) {
					if (!pgui->audioHandle.stream.stop()) {
						...
						MessageBox(hwnd, L"Unable to stop the stream.", L"Error", MB_ICONERROR | MB_OK);
					}
				}
			}
		}
	...
	case WM_CLOSE: {
		GUI* pgui = (GUI*)GetWindowLongPtr(hwnd, GWLP_USERDATA);
		if (!pgui->closeWAVStream()) {
			MessageBox(hwnd, L"Unable to close the stream.", L"Error", MB_ICONERROR | MB_OK);
		}
		DestroyWindow(hwnd);
	}
	break;
